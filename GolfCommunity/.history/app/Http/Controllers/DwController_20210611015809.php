...
class DwController extends Controller
{
    ...
    public function tambahsaldobaru(Request $request){
        ...
        public function transfer_proses(Request $request){
            DB::table('digital_wallets')->where('id_user',$request->id_user)->decrement('saldo', $request->saldo);
            
            DB::table('historidw')->insert([
                'id_user' => $request->id_user,
                'catatan' => "Transfer sebesar Rp" . $request->saldo . " ke " . $request->catatan,
                'nominal' => '- Rp' . $request->saldo,
                'tgl' => date('Y-m-d')
            ]);
            return redirect('/digitalwallet');
