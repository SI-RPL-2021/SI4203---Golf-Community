...
class TiketController extends Controller
{
    ...
    public function hasiltiket(Request $request)
    {
        ...
        DB::table('digital_wallets')->where('id_user',$request->id_user)->decrement('saldo', $request->total_harga);
            
        DB::table('historidw')->insert([
            'id_user' => $request->id_user,
            'catatan' => "Membeli Tiket Pertandingan dengan kode tiket: " . $request->kode,
            'nominal' => '- Rp' . $request->total_harga,
            'tgl' => date('Y-m-d')
        ]);
